...
app.get('/games/:id', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`SELECT g.id, g.name, p.name AS platform_id, genre, mature, price, rating
    FROM games AS g
    JOIN gamingPlatform AS p
    ON g.platform_id = p.id
    WHERE g.id=$1
  ...
  try {
    ...
    const data = await client.query(`
        UPDATE games
          ...
          WHERE games.id = $7
          RETURNING *
      `,  [updatedGameList.name, updatedGameList.genre, updatedGameList.mature,
      updatedGameList.price, updatedGameList.rating, updatedGameList.platform_id, gameId]);
...
app.post('/games', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      INSERT INTO games(name, platform_id, genre, price, rating, image)
      VALUES($1, $2, $3, $4, $5, $6)
      RETURNING * 
      `, [newGame.name, newGame.platform_id, newGame.genre, newGame.price, newGame.rating, newGame.image]);
